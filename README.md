...
posting online — while the cat is lying still and silent under a deck four houses away.
...
| Finding | Figure |
|---|---|
| Indoor-only cats: distance from escape point when found | **median 39 m** (IQR 9–137 m) |
...
| Recovered alive by day 7 | 34% (95% CI 31–37%) |
| Recovered alive within one year | 61% (95% CI 57–64%) |
...
1. **A field protocol** — [`docs/protocol/`](docs/protocol/). What to do, in order, starting the
...
Greenfield. Nothing is deployed and there is no Supabase project yet. Current contents are
documentation, architecture decisions, and a first schema.

The protocol documents are usable now and are the part most likely to matter to someone before
any code ships. If you only read one thing here, read
[`docs/protocol/first-24-hours.md`](docs/protocol/first-24-hours.md).

## Planned stack

Supabase — Postgres with PostGIS for the search grid and sighting geography, Auth, Storage,
Realtime for shared searching, Edge Functions, and row-level security doing the privacy work.
